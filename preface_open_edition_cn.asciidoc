...
((("code examples, obtaining and using"))所有的程序示例都是用Solidity，Vyper和JavaScript写成的，并在类似Unix的操作系统的命令行下进行操作。所有代码均位于GitHub代码库 _code_ 子目录下。你可以复制书里面的代码，测试它们，并通过GitHub提交更正：https://github.com/ethereumbook/ethereumbook.
