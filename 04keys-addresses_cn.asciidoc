...
((("one-way functions"))) 简单来说，哈希函数http://bit.ly/2CR26gD[_hash function_]是可用于将任意大小的数据映射到固定大小的数据的任何函数。((("pre-image")))哈希函数的输入称为 _pre-image_ ， _message_ 或简称为 _input data_ 。输出称为 _hash_ 。 密码学哈希函数 http://bit.ly/2Jrn3jM[_Cryptographic hash functions_]是一个特殊的子类别，具有特定的属性，这些属性可用于保护以太坊等平台。
