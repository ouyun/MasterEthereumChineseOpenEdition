...
[[sign_function]]
函数 _F_~_sig_~生成签名__Sig__，该签名由两个值组成，通常称为 _r_和_s_：
...
((("digital signatures","ECDSA math", id="ix_06transactions-asciidoc16", range="startofrange")))((("Elliptic Curve Digital Signature Algorithm (ECDSA)","mathematics of", id="ix_06transactions-asciidoc17", range="startofrange"))) 如前所述，签名是由数学函数 _F_〜_sig_〜创建的，该函数生成由两个值 _r_和_s_组成的签名。在本节中，我们将详细介绍函数 _F_〜_sig_〜。
...
* 从 _q_和椭圆曲线生成器点_G_生成的对应的临时公钥_Q_
...
验证是签名生成函数的逆函数，使用 _r_和_s_值以及发送者的公钥来计算值_Q_，该值是椭圆曲线上的点（签名创建中使用的临时公钥）。步骤如下：

1. 检查所有输入的格式是否正确
2. 计算 _w_ = __s__^-1^ _mod p_
3. 计算 _u~1~_ = __Keccak256 __（__ m__）* _w_ _mod p_
...
如果所计算的点 _Q_的_x_坐标等于_r_，则验证者可以得出签名有效的结论。
...
* _s_ 是签名的 +s+ 值。
* __R__和 __R __^'^是临时公钥 _Q_ 的两种可能性。
