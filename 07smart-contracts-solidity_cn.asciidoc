...
((("Truffle","running test transaction with"))) 让我们使用 +truffle+ 在修订后的 +Faucet+ 合约上运行测试交易。请按照 <<truffle>> 设置项目目录并编译 pass:[ <span class="keep-together"><code>Faucet</code></span> ]代码。可以在本书的GitHub代码库 https://github.com/ethereumbook/ethereumbook[the book's GitHub repository] 的目录_code / truffle / FaucetEvents_下找到源代码。
