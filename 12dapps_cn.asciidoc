...
image::{abspath}images/web3suite.png["Web3: A decentralized web using smart contracts and P2P Technologies"]
...
智能合约体系结构设计的一个主要考虑因素是，一旦部署，便无法更改智能合约的代码。如果使用可访问的 +SELFDESTRUCT+ 操作码对其进行编程，则可以将其删除，但是除了完全删除以外，不能以任何方式更改该代码。
...
((("DApps (decentralized applications)","frontend (web user interface)")))((("user interface, as DApp frontend")))((("web user interface, as DApp frontend")))与DApp的业务逻辑不同（要求开发人员理解EVM和诸如Solidity之类的新语言），DApp的客户端界面可以使用标准的Web技术（HTML，CSS，JavaScript等）。这使传统的Web开发人员可以使用熟悉的工具，库和框架。与以太坊的交互（例如，签名消息，发送交易和管理密钥）通常通过Web浏览器，通过诸如MetaMask之类的扩展程序进行（请参见<<intro_chapter>>）。
...
((("DApps (decentralized applications)","decentralized message communication protocols")))((("message communication protocols")))应用程序的另一个主要组成部分是进程间通信。这意味着能够在应用程序之间，应用程序的不同实例之间或应用程序的用户之间交换消息。传统上，这是依赖中央服务器来实现的。但是，对于基于服务器的协议，有许多去中心化的替代方法，它们可通过P2P网络提供消息传递。 ((("Whisper"))) DApp最著名的P2P消息协议是 http://bit.ly/2CSls5h[_Whisper_] ，它是以太坊基金会Go-Ethereum工具组件的一部分。
...
.拍卖DApp：一个简单的拍卖DApp示例
image::{abspath}images/auction_diagram.png["Auction DApp: A simple example auction DApp"]

你可以 http://bit.ly/2DcmjyA[在本书的存储库中] 找到拍卖DApp的源代码 。
...
让我们开始查看 +DeedRepository+ ，如<<deed_repository_code>>。该合约是与ERC721兼容的非同质化通证（请参见<<erc721>>）。
...
我们的拍卖DApp使用 +DeedRepository+ 合约为每次拍卖发行和跟踪通证。拍卖本身由 +AuctionRepository+ 合约安排。该合约太长，无法在此处完整列出，但<<auction_repository_code>>显示合约和数据结构的主要定义。本书的 https://bit.ly/2IaOo9i[GitHub存储库] 中提供了整个合约。
...
这是一个明智的选择，它可以分散DApp的管理权，并在部署后放弃任何控制。相比之下，某些DApp具有一个或多个特权帐户，这些帐户具有特殊功能，例如终止DApp合约，覆盖或更改其配置或“否决”某些操作的能力。通常，在DApp中引入了这些治理功能，以避免由于错误而引起的未知问题。
...
您可以在 https://github.com/ethereumbook/ethereumbook[这本书的存储库] 的 _code/auction_dapp/frontend_ 文件夹中找到用户界面代码。该目录具有以下结构和内容：
...
[[auction_dapp_screen]]
.拍卖DApp的用户界面
image::{abspath}images/auction_dapp_home_screen.png["Auction DApp User interface"]
...
任何人都可以通过手动构建交易或在其本地计算机上运行Vue前端来与DApp进行交互。 DApp代码本身是开源的，并在公共仓库上协同开发。
...
((("Auction DApp","storing on Swarm", id="ix_12dapps-asciidoc6", range="startofrange")))((("Swarm","storing Auction DApp on", id="ix_12dapps-asciidoc7", range="startofrange")))我们在本章前面<<swarm_sec>>中介绍过Swarm。我们的Auction DApp已经使用Swarm来存储每次拍卖的图标图像。这比尝试直接在以太坊上存储数据要有效得多。与将这些图像存储在Web服务器或文件服务器等集中式服务中相比，它具有更大的弹性。

但是，我们可以采取进一步的行动。我们可以将DApp本身的整个前端存储在Swarm中，并直接从Swarm节点运行它，而不是运行Web服务器。

==== 准备Swarm

((("Swarm","installing and initializing"))) 要开始，你需要安装Swarm并初始化Swarm节点。 Swarm是以太坊基金会Go-Ethereum工具套件的一部分。请参阅<<go_ethereum_geth>>中安装Go-Ethereum的说明，或者安装Swarm发布的可执行版，请按照 http://bit.ly/2Q75KXw[Swarm文档] 中的说明进行操作。
...
要开始运行Swarm，你必须告诉它如何连接到Geth实例，以访问JSON-RPC API。请按照 https://swarm-guide.readthedocs.io/en/latest/gettingstarted.html[入门指南]中的说明进行操作。
...
[[image_swarm_gateway]]
.localhost上的swarm网关
image::{abspath}images/swarm-gateway.png["Swarm gateway on localhost"]

==== 上传文件到Swarm

((("Swarm","uploading files to", id="ix_12dapps-asciidoc8", range="startofrange"))) 一旦运行了本地Swarm节点和网关，就可以上传到Swarm，文件将只需简单地参考其哈希即可在任何Swarm节点上访问。
...
Swarm已上传 _README.md_ 文件，并返回了一个哈希，你可以使用该哈希从任何Swarm节点访问该文件。例如，你可以使用 https://bit.ly/2znWUP9[公共Swarm网关]。
...
((("ENS (Ethereum Name Service)","history of")))名字注册是Namecoin率先开发的区块链的第一个非货币应用程序。以太坊 http://bit.ly/2Of1gfZ[白皮书] 中提供了两行的类似于Namecoin的注册系统，作为其示例应用程序之一。

Geth的早期版本和C++ Ethereum客户端具有内置的 +namereg+ 合约（不再使用），并且提出了许多有关名称服务的提案和ERC，但直到 Nick Johnson 于 2016 年开始为以太坊基金会工作并将该项目置于他的领导之下时，注册商的认真工作才开始。
...
Namehash递归地对名称的组成部分进行哈希处理，从而为任何有效的输入域生成唯一的，固定长度的字符串（或&#x201c;节点&#x201d;）。例如，Namehash节点 +subdomain.example.eth+ 是 `keccak('<example.eth>' node) + keccak('<subdomain>')`。我们必须解决的子问题是为 +example.eth+ 计算节点，即 `keccak('<.eth>' node) + keccak('<example>')`。首先，我们必须为 +eth+ 计算节点，即`keccak(<root node>) + keccak('<eth>')`。
...
全部放在一起，因此节点 +subdomain.example.eth+ 是 pass:[<code><span class="keep-together">keccak</span>(keccak(keccak(0x0...0 + keccak('eth')) + keccak('example')) + keccak('subdomain'))</code>]。
...
===== 如何选择一个有效的名字
((("ENS (Ethereum Name Service)","choosing a valid name"))) 名称由一系列点分隔的标签组成。尽管允许使用大写和小写字母，但所有标签都应遵循UTS &#x23;46 规范化过程，该过程会在对标签进行哈希处理之前对标签进行大小写合并，因此，具有不同大小写但相同拼写的名称将以相同的Namehash结尾。
...
((("ENS (Ethereum Name Service)","root node ownership"))) 此分层系统的结果之一是，它依赖于能够创建顶级域的根节点所有者（TLDs）。

虽然最终目标是为新TLDs采用分布式的决策生成过程，但在撰写本文时，根节点由不同国家/地区的人们持有的4/7多重签名控制（反映了7个DNS系统的主要所有者）。
结果，需要7个密钥持有者中至少4个的大多数才能进行任何更改。

目前，这些主要负责人的目的和目标是与社区达成共识，以：

* 一旦对系统进行评估完成，就将 +.eth+ TLD的临时所有权迁移并升级为更永久的合约。
* 如果社群同意需要，则允许添加新的TLDs。
...
(((".eth nodes")))((("ENS (Ethereum Name Service)","middle layer: .eth nodes")))((("eth nodes"))) 在撰写本文时，在智能合约中唯一可注册的顶级域是 +.eth+ 。
...
* 由于你无法在去中心化系统中同时披露所有出价，因此投标人必须自己披露自己的出价；如果没有，他们将没收他们的锁定资金。没有这种损失，一个人可能会出价很多次，只选择披露一个或两个，就把一次密封的竞标变成了传统的涨价 pass:[ <span class="keep-together">拍卖</span> ]。
...
((("deed","as ENS top layer")))((("ENS (Ethereum Name Service)","top layer: deeds")))ENS的顶层是另一个超级简单的合约，其目的只有一个：持有资金。

当您赢得一个名字时，资金实际上不会发送到任何地方，而只是在您想要持有该名字的时间段内（至少一年）被锁定。
这就像一个有保证的回购：如果所有者不再想要这个名字，他们可以把它卖回系统并收回他们的以太（所以持有这个名字的成本是做某事回报大于零的机会成本 ）。
...
((("ENS (Ethereum Name Service)","registering a name", id="ix_12dapps-asciidoc14", range="startofrange")))((("registering a name", id="ix_12dapps-asciidoc15", range="startofrange"))) 在ENS中注册名称是一个四步过程，正如我们在<<vickrey_auction>>所见。首先，我们对任何可用的名称进行出价，然后在48小时后披露出价以确保名称安全。 <<ens_registration_timeline>>是显示注册时间线的图。

让我们注册我们的第一个名字！

我们将使用几个可用的用户友好界面之一来搜索可用的名称，对名称 +ethereumbook.eth+ 出价，披露出价并确定名称。

ENS有许多基于Web的界面，使我们可以操作ENS DApp。在此示例中，我们将使用 https://mycrypto.com/[MyCrypto接口]，并将MetaMask用作钱包。

[[ens_registration_timeline]]
.ENS注册时间线
image::{abspath}images/ens-flow.png["ENS注册时间线"]
...
image::{abspath}images/ens-checkname.png["Searching for ENS names on MyCrypto.com"]
...
[[ens-auction]]
.开始拍卖一个ENS名称
image::{abspath}images/ens-auction.png["Starting an auction for an ENS name"]
...
[[ens-bid]]
.竞标ENS名称
image::{abspath}images/ens-bid.png["Placing a bid for an ENS name"]

[WARNING]
====
如在<<vickrey_auction>>中提及，你必须在拍卖完成后的48小时内披露你的出价，否则就会 _损失出价中的资金_。我们是否忘记这样做并自己损失了0.01 ETH？肯定不会啊。
...
[[ens-metamask-bid]]
.MetaMask交易包含你的出价
image::{abspath}images/ens-metamask-bid.png["MetaMask transaction containing your bid"]
...
一旦打开这个网站，在搜索框中输入要管理的名称（请参见<<ens-manager>>）。你需要解锁以太坊钱包（例如MetaMask），以便ENS Manager DApp可以代表你管理名称。

[[ens-manager]]
.ENS网络管理界面
image::{abspath}images/ens-manager.png["ENS网络管理界面"]
...
image::{abspath}images/ens_manager_add_subdomain.png["Adding the subdomain auction.ethereumbook.eth"]
...
image::{abspath}images/ens-manager-set-default-resolver.png["Set the default public resolver for auction.ethereumbook.eth"]
...
image::{abspath}images/ens-manager-set-content.png["Set the 'content' return for auction.ethereumbook.eth"]
...
.拍卖DApp的架构
image::{abspath}images/auction_dapp_final_architecture.png[“拍卖DApp的架构”]

=== 本章小结

正如早期设计的创始人所表达的那样，去中心化应用程序是以太坊愿景的顶点。 虽然今天很多应用程序都称自己为“DApps”，但大多数应用程序并没有完全去中心化。 然而，构建几乎完全去中心化的应用程序已经成为可能。 随着时间的推移，随着技术的进一步成熟，我们越来越多的应用程序可以去中心化，从而形成一个更具弹性、抗审查性和免费的网络。(((range="endofrange", startref="ix_12dapps-asciidoc0")))