...
[TIP]
====
对&#926;使用Unicode字符 +U+039E+ ，对&#9830;使用 +U+2666+ 。
====
...
<<ether_denominations>>显示各种单位，其俗语（通用）名称及其SI名称。为了与价值的内部表示保持一致，该表在显示所有面值时以 wei 为单位（第一行），在第7行中以太显示为10^18^ wei。
...
* 请勿在数字文档，数字照片，屏幕截图，在线驱动器，加密的PDF等文件中存储任何密码。同样，请勿即兴地提高安全性。使用密码管理器或笔和纸。
...
* 创建新帐户时，首先仅将一小笔测试交易发送到新地址。收到测试交易后，请尝试再次从该帐户发送回。帐户创建有很多原因可能会出错，如果出错，以一个很小的损失为代价总归好于大的损失。如果测试正常，一切都很好。
...
image::{abspath}images/metamask_download.png["MetaMask详细页面"]
...
image::{abspath}images/metamask_password.png["MetaMask密码页面"]
...
[[metamask_mnemonic]]
.由MetaMask创建的钱包助记符备份
image::{abspath}images/metamask_mnemonic.png["MetaMask助记符页面"]

确认安全存储了助记符后，您将能够查看以太坊帐户的详细信息，如<<metamask_account>>.(((range="endofrange", startref="ix_02intro-asciidoc5")))

[[metamask_account]]
.您在MetaMask中的以太坊账户
image::{abspath}images/metamask_account.png["MetaMask帐户页面"]
...
Localhost 8545:: 连接到与浏览器在同一台计算机上运行的节点。该节点可以是任何公共区块链（主网或某测试网）的一部分，也可以是私有测试网的一部分。
...
((("ether (generally)","testnet")))((("MetaMask","and testnet ether")))((("test ether","obtaining")))((("testnet","ether for")))((("wallets","testnet ether and")))你的首要任务是为你的钱包充值。你不用在主网络上执行此操作，因为真正的以太坊需要花费金钱，而处理它需要更多的经验。下面将介绍你如何使用一些测试网的 ether 来为你的钱包充值。
...
image::{abspath}images/metamask_ropsten_faucet.png["MetaMask Ropsten测试水龙头"]
...
单击绿色的“从水龙头请求1以太”按钮。您会看到交易ID出现在页面的下部。水龙头应用程序创建了一笔交易&#x2014;向您付款。交易ID如下所示：
...
image::{abspath}images/ropsten_block_explorer.png["Etherscan Ropsten Block Explorer"]
...
[[send_to_faucet]]
.向水龙头发送1个以太币
image::{abspath}images/send_to_faucet.png["向水龙头发送1个以太币"]
...
.MetaMask帐户交易记录菜单
image::{abspath}images/metamask_account_context_menu.png["MetaMask帐户记录菜单"]
...
image::{abspath}images/block_explorer_account_history.png["Etherscan上的账户公开地址交易历史记录"]
...
EVM是全局单例，这意味着它可以像运行在各处的全局单实例计算机一样运行。以太坊网络上的每个节点都运行EVM的本地副本以验证智能合约的执行，而以太坊区块链则记录了该世界计算机在处理交易和智能合约时的变化状态。我们将在<<evm_chapter>>讨论它的更多细节。
...
请注意，由于合约帐户没有私钥，因此它无法直接启动一个交易。只有EOA可以启动交易，但是合约可以通过调用其他合约来构建交易，从而构建复杂的执行路径。 EOA的一种典型用法是EOA将请求交易发送到多签名智能合约钱包，以将ETH发送到另一个地址。典型的DApp编程模式是让合约 A调用合约 B，以便在合约 A的用户之间维持共享状态。
...
((("Solidity","faucet.sol and")))对于我们的第一个示例（<<solidity_faucet_example>>），我们将写一个控制 _水龙头_ 的合约。你已经使用过水龙头在Ropsten测试网络上获取测试以太币ether。水龙头是一个相对简单的合约：它向任何请求提现的账户地址发出以太币，并且可以不时重新充值。你可以将水龙头改造为由人工或网页服务器控制的钱包。
...
该行声明了一个合约 +contract+ 对象，类似于其他面向对象的语言中的 +class+ 声明。合约定义包括花括号（pass:[<code>{}</code>]）之间的所有行，它们定义了一个 _scope_ ，就像在许多其他编程语言中如何使用花括号一样。
...
该函数名为提现 +withdraw+ ，它采用一个无符号整数（ +uint+ ）参数名为 +withdraw_amount+ 。它被声明为公共功能函数，这意味着它可以被其他合约调用。函数定义在花括号之间。+withdraw+ 函数的第一部分设置了提现的金额限制：
...
这里发生了一些有趣的事情。 +msg+ 对象是所有合约均可访问的输入之一。它代表触发该合约执行的交易。属性 +sender+ 是交易的发送者地址。函数 +transfer+ 是一个内置函数，可将以太币ether从当前合约转移到发送者的地址。从后往前看的话，这意味着 +transfer+ 这个 +msg+ 到 +sender+ 会触发该合约的执行。 +transfer+ 函数将一个数量作为唯一参数。我们将 +withdraw_amount+ 值作为参数传递给前面几行声明的 +withdraw+ 函数。
...
我们的默认功能正下方是最后一个大括号，它关闭了合约 +Faucet+ 的定义。就是这样！(((range="endofrange", startref="ix_02intro-asciidoc11")))(((range="endofrange", startref="ix_02intro-asciidoc10")))
...
[[remix_close_tab]]
.关闭默认示例选项
image::{abspath}images/remix_close_tab.png["关闭默认示例选项"]

现在，通过单击左上方工具栏中的圆形加号来添加新标签，如<<remix_toolbar>>。将新文件命名为 _Faucet.sol_。

[[remix_toolbar]]
.单击加号打开一个新选项
image::{abspath}images/remix_toolbar.png["单击加号打开一个新选项"]

打开新标签后，请复制并粘贴示例 _Faucet.sol_ 中的代码，如 <<remix_faucet_load>>。

[[remix_faucet_load]]
.将水龙头示例代码复制到新的选项卡中
image::{abspath}images/remix_faucet_load.png[“将水龙头示例代码复制到新的选项卡中”]

将 _Faucet.sol_ 合约加载到Remix IDE中之后，IDE将自动编译代码。如果一切顺利，你将在右侧的“编译”选项卡下看到一个带有“水龙头”的绿色框，确认编译成功（请参见<<remix_compile>>）。

[[remix_compile]]
.Remix成功编译了Faucet.sol合约
image::{abspath}images/remix_compile.png["Remix成功编译了Faucet.sol合约"]
...
image::{abspath}images/remix_run.png["在Remix IDE的“Run”选项中，选择“Injected Web3”环境"]
...
.MetaMask显示合约创建过程的交易
image::{abspath}images/remix_metamask_create.png["MetaMask显示合约创建过程的交易"]
...
.水龙头合约上线了！
image::{abspath}images/remix_contract_interact.png["水龙头合约上线了!"]
...
.从Remix复制合约地址
image::{abspath}images/remix_contract_address.png["从Remix复制合约地址"]
...
image::{abspath}images/etherscan_contract_address.png["在etherscan块浏览器中查看水龙头合约的地址"]
...
.将1个以太币发送到合约地址
image::{abspath}images/metamask_send_to_contract.png["将1个以太币发送到合约地址"]
...
当你将交易发送到合约地址时，没有数据指定要调用的函数，它将调用此默认函数。因为我们将其声明为 +payable+，所以它接受了1个以太币并将其存入合约的帐户余额中。你的交易将会在EVM的合约中运行，从而更新了余额。你已经完成了水龙头合约的充值！(((range="endofrange", startref="ix_02intro-asciidoc18")))
...
image::{abspath}images/remix_contract_interact.png["在Remix中Faucet.sol的提现功能"]
...
image::{abspath}images/remix_withdraw.png[“在Remix中单击“提现”以创建提现交易”]

MetaMask将弹出一个交易窗口供您批准。点击确认，将你的提现通知发送到合约（请参见<<metamask_withdraw>>）。

[[metamask_withdraw]]
.MetaMask调用提现功能的交易
image::{abspath}images/metamask_withdraw.png["MetaMask调用提现功能的交易"]
...
image::{abspath}images/etherscan_withdrawal_tx.png["Etherscan显示调用提现功能的交易"]


现在，我们看到一个新的交易，目的地为合约地址，交易金额为0 ether。合约的余额已更改，现在为0.9以太，因为它已按要求向我们发送了0.1以太。但是，我们在 _合约地址历史记录_ 中看不到“OUT”交易。

那么这个提现的操作到哪里去了？合约的地址历史记录页面上出现了一个新标签，名为内部交易。 ((("internal transaction (message)")))由于源自合约代码的0.1 ether转移是内部交易（也称为 _message_ ）。点击该标签以查看它（请参见<<etherscan_withdrawal_internal>>）。
...
image::{abspath}images/etherscan_withdrawal_internal.png[“Etherscan显示将以太币从合约中转出的内部交易”]
